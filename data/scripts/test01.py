...
# with open('demo/test/_annotations.coco.json', 'r') as f:
#     data = json.load(f)
#     print(data)
    ...
    for annotation in json_data['annotations']: # 遍历标注数据信息
        ...
        for image in json_data['images']: # 遍历图片相关信息
            if image['id'] == image_id:
                ...
                txt_file_name = os.path.splitext(image['file_name'])[0]+ '.txt'
                break
...
if __name__=="__main__":
    img_dir = 'demo/test/'
    target_dir = 'demo/test_yolo/'
    if not os.path.exists(target_dir):
        os.mkdir(target_dir)
    in_json_path = 'demo/test/_annotations.coco.json'
