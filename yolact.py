    ...
    def forward(self, x):
        ...
        if cfg.fpn is not None:
            with timer.env('fpn'):
                ...
                outs = self.fpn(outs)   #! fpn 有5个尺寸的输出
        # return outs  #!     
        proto_out = None
        ...
        for k, v in pred_outs.items():
            pred_outs[k] = torch.cat(v, -2) #! 将5个层的输出结果拼接在一起 mask conf: torch.Size([1, 19248, 32])
        ...
        if self.training:
            ...
            if cfg.use_focal_loss:
                ...
                if cfg.use_objectness_score:
                    ...
                    pred_outs['conf'] = F.softmax(pred_outs['conf'], -1)
            #! mask conf: torch.Size([1, 19248, 32])
            out = self.detect(pred_outs, self)
            return out
...
if __name__ == '__main__':
    import sys
    sys.path.append('/home/aizhiqing/mnt/aizhiqing/python_code/python_projects/yolact')
    ...
    net = Yolact()
    # net.train()
    # # net.init_weights(backbone_path=cfg.backbone.path)
    # net.init_weights(backbone_path = '/home/aizhiqing/mnt/aizhiqing/python_code/python_projects/yolact/yolact_resnet50_54_800000.pth')
    save_model  = './test.onnx'
    # GPU
    net = net.cuda()
    torch.set_default_tensor_type('torch.cuda.FloatTensor')

    x = torch.zeros((1, 3, cfg.max_size, cfg.max_size))
    input_shape =(1, 3, cfg.max_size, cfg.max_size) 
    ...
    torch.onnx.export(
            net,  # pytorch网络模型
            input,    # 随机的模拟输入
            save_model,     # 导出的onnx文件位0置
            # export_params=True,     # 导出训练好的模型参数
            verbose=False,             # debug message
            # training=2,
            opset_version=11,  # onnx version11以上，将upsample转换为resize，完全匹配pytorch；但是出现constant，不支持部署
    ...
    # flops, params = profile(model, input_shape)
    print(" %.2fM | %.2fG" % ( params / (1000 ** 2), flops / (1000 ** 3)))
    
    import onnx
    from onnx import shape_inference
    onnx.save(shape_inference.infer_shapes(onnx.load(save_model)), save_model)   
