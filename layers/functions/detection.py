...
class Detect(object):
    ...
    def __init__(self, num_classes, bkg_label, top_k, conf_thresh, nms_thresh):
        self.num_classes = num_classes  #! 加上了背景类，对于coco ,则是80+1
    ...
    def __call__(self, predictions, net):
        ...
        with timer.env('Detect'):
            ...
            for batch_idx in range(batch_size):
                decoded_boxes = decode(loc_data[batch_idx], prior_data) #! bbox解码，根据预测的偏移量和先验框解码出预测框
                result = self.detect(batch_idx, conf_preds, decoded_boxes, mask_data, inst_data)    #! 执行NMS， 得到top_k个输出
                ...
                """
                #! nms后，这里shape值为：
                result['proto'].shape-> torch.Size([138, 138, 32]);
                result['box'].shape-> torch.Size([100, 4]);
                result['mask'].shape-> torch.Size([100, 32]);
                result['class'].shape-> torch.Size([100]);
                result['score'].shape-> torch.Size([100]);
                """
