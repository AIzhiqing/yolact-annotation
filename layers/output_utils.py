...
def postprocess(det_output, w, h, batch_idx=0, interpolation_mode='bilinear',
    ...
    if score_threshold > 0:
        keep = dets['score'] > score_threshold  #! 保留大于阈值的检测框，NMS后做第二次过滤
    ...
    if cfg.mask_type == mask_type.lincomb and cfg.eval_mask_branch:
        ...
        if crop_masks:
            masks = crop(masks, boxes)  #! 裁剪掉bbox外部的掩膜

        # Permute into the correct output shape [num_dets, proto_h, proto_w]
        masks = masks.permute(2, 0, 1).contiguous() #! 将proto原型和 mask conf 线性组合得到分割结果
        ...
        # Binarize the masks
        #! 为 masks 的张量执行 masks.gt_(0.5) 时，这个操作会检查 masks 中的每个元素，并将所有大于0.5的元素的值设置为 True（在PyTorch中，True 对应于整数 1），
        #! 而所有小于或等于0.5的元素的值将被设置为 False（对应于整数 0）
        masks.gt_(0.5)
